...
def get_user_data_script(port):
    """Build the EC2 user-data script that installs and starts the HTTP API chat server."""
    return f"""#!/bin/bash
set -e
...
# Write server.js (HTTP API - Lambda relays WebSocket traffic)
...
function fetchUserProfile(token) {{
  return new Promise((resolve, reject) => {{
    ...
    const options = {{ hostname: url.hostname, path: url.pathname, method: 'GET', headers: {{ Authorization: token }} }};
...
const httpServer = http.createServer(async (req, res) => {{
  ...
  if (req.url === '/health' && req.method === 'GET') {{
    respond(res, 200, {{ status: 'ok', service: 'eemployee-chat', version: '3.0', port: PORT, rooms: Object.keys(rooms).length }});
    return;
  }}

  if (req.url === '/auth' && req.method === 'POST') {{
    const body = await readBody(req);
    if (!body.token) {{ respond(res, 400, {{ error: 'token required' }}); return; }}
    try {{
      const decoded = await verifyToken(body.token);
      const profile = await fetchUserProfile(body.token);
      if (!profile || !profile.orgId) {{ respond(res, 401, {{ error: 'User profile not found' }}); return; }}
      respond(res, 200, {{ email: decoded.email || profile.email, displayName: profile.displayName || decoded.email, orgId: profile.orgId }});
    }} catch {{ respond(res, 401, {{ error: 'Invalid token' }}); }}
    return;
  }}
  ...
  if (req.url === '/join' && req.method === 'POST') {{
    ...
    if (!orgId || !email) {{ respond(res, 400, {{ error: 'orgId and email required' }}); return; }}
    for (const rid of Object.keys(rooms)) {{ rooms[rid].delete(email); if (rooms[rid].size === 0) delete rooms[rid]; }}
    if (!rooms[orgId]) rooms[orgId] = new Set();
    rooms[orgId].add(email);
    userMeta[email] = {{ email, displayName, orgId }};
    const userList = [];
    for (const e of rooms[orgId]) {{ if (userMeta[e]) userList.push({{ email: e, displayName: userMeta[e].displayName }}); }}
  ...
  if (req.url === '/leave' && req.method === 'POST') {{
    ...
    if (!orgId || !email) {{ respond(res, 400, {{ error: 'orgId and email required' }}); return; }}
    if (rooms[orgId]) {{ rooms[orgId].delete(email); if (rooms[orgId].size === 0) delete rooms[orgId]; }}
  ...
  if (req.url === '/message' && req.method === 'POST') {{
    ...
    if (!orgId || !from) {{ respond(res, 400, {{ error: 'orgId and from required' }}); return; }}
    respond(res, 200, {{ broadcast: {{ type: 'message', from, payload, iv, timestamp: new Date().toISOString() }} }});
    return;
  }}
...
httpServer.listen(PORT, '0.0.0.0', () => {{
  console.log(`E-Employee Chat Server v3.0 (HTTP API) on port ${{PORT}}`);
}});
SERVERJS
